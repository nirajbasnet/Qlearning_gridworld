...
def try_move(agent, action_cmd):
    global player,episode,targets,characters, score, walk_reward, me,me2,target, restart , moves,moves_sp,score_sp,sum_moves
    player_id=0
    target_id=1
    ...
    for (i, j) in targets:
        ...
        if player[0][0] == i and player[0][1] == j:
            ...
            sum_moves+=moves
            if score > 0:
                print("epi:", episode, " Success! score: ", score, "moves:",sum_moves)
            else:
        ...
        if player[1][0] == i and player[1][1] == j:
            # episode += 1
            score_sp -= walk_reward
            score_sp += 1
            if score_sp > 0:
                print("")
                #print("epi:", episode, " Success! score_sp: ", score_sp, "moves:",moves_sp/2.0)
            else:
                print("Fail! score: ", score_sp)
            moves_sp=0
            moves=0
            restart = True
            return
...
def restart_game():
    ...
    player[0]=(random.randint(0,x),random.randint(0,y))

    player_home[0]=player[0]
    #player[1]=(random.randint(0,x),random.randint(0,y))
    player[1]=player[0]
    player_home[1] = player[1]

    print("home", player[0])
    print("home2", player[1])
    targets[0]=target_home[0]
    if (player[0]==targets[0]):
        restart_game()
    score = 1
    score_sp=1
