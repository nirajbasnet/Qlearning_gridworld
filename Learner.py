...
class Qlearning:
    ...
    def do_action_sp(self,action):
        sp = World.player[0]
        r = -World.score_sp
        World.try_move(World.characters[1], action)
        sp2 = World.player[1]
        r += World.score_sp
        return sp, action, r, sp2

    def valid_actions(self,s):
        valid_acts=[]
        for a in World.actions:
            dx,dy=World.map_action_commands(a)
            newx,newy=s[0]+dx,s[1]+dy
            if newx<World.x and newx>=0:
                if newy<World.y and newy>=0:
                    valid_acts.append(a)
        return valid_acts
    ...
    def get_qvalue_sp(self,s,a):
        dx, dy = World.map_action_commands(a)
        new_s=(s[0]+dx,s[1]+dy)
        dist=self.manhattandistance(World.targets[0],new_s)
        sum_feature=self.weights_sp[0]*self.bias+self.weights_sp[1]*dist
        return sum_feature
    ...
    def find_best_action(self,state):
        if(random.random()<self.epsilon and self.train_episodes<100):
            act=random.choice(World.actions)
        else:
            act,val=self.max_Q(state)
        return act

    def find_best_action_sp(self,state):
        if(random.random()<self.epsilon and self.train_episodes<100):
            act=random.choice(World.actions)
        else:
            act,val=self.max_Q_sp(state)
        return act
    ...
    def run(self):
        ...
        while True:
            ...
            max_act_sp=self.find_best_action_sp(sp)
            s, a, r, s2 = self.do_action(max_act)
            sp,a_sp,r_sp,sp2=self.do_action_sp(max_act_sp)

            total_reward=r+r_sp
            if self.train_episodes<100:
                # Update Q
                max_act, max_val = self.max_Q(s2)
                difference = (total_reward + self.discount * max_val) - self.get_qvalue(s,a)
                ...
                max_act_sp, max_val_sp = self.max_Q_sp(sp2)
                difference = (total_reward + self.discount * max_val_sp) - self.get_qvalue_sp(sp2, a_sp)
                self.weights_sp[0] += self.alpha * difference * 1.0  # for bias feature
                self.weights_sp[1] += self.alpha * difference * self.manhattandistance(World.targets[0],
                                                                                    sp2)  # for distance feature
